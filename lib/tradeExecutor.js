...
async function isDailyDrawdownBreached(accountValue) {
  if (!accountValue || accountValue <= 0 || DAILY_DRAWDOWN_LIMIT_PCT <= 0) {
    return false;
  }

  const today = new Date().toISOString().split('T')[0];
  const drawdownRef = doc(db, 'settings', 'daily_drawdown');

  try {
    const drawdownDoc = await getDoc(drawdownRef);
    let dayStartAccountValue = accountValue;

    if (drawdownDoc.exists()) {
      const data = drawdownDoc.data();
      if (data.date === today) {
        dayStartAccountValue = data.startAccountValue;
      } else {
        await setDoc(drawdownRef, { date: today, startAccountValue: accountValue });
      }
    } else {
      await setDoc(drawdownRef, { date: today, startAccountValue: accountValue });
    }

    const drawdownPct = (dayStartAccountValue - accountValue) / dayStartAccountValue;
    if (drawdownPct >= DAILY_DRAWDOWN_LIMIT_PCT) {
      console.warn(`⚠️ Daily drawdown limit breached: ${(drawdownPct * 100).toFixed(2)}% (limit: ${(DAILY_DRAWDOWN_LIMIT_PCT * 100).toFixed(0)}%). Forcing HOLD on all positions.`);
      return true;
    }
  } catch (error) {
    console.error('❌ Error checking daily drawdown:', error.message);
  }
...
export async function executeAutoTrades({ signal, marketData, account }) {
  ...
  // Check daily drawdown limit before executing any trades
  const drawdownBreached = await isDailyDrawdownBreached(accountValue);
  if (drawdownBreached) {
    return { executed: false, reason: `Daily drawdown limit of ${(DAILY_DRAWDOWN_LIMIT_PCT * 100).toFixed(0)}% breached - all positions forced to HOLD` };
  }
