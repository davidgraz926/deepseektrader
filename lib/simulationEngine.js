...
import { DAILY_DRAWDOWN_LIMIT_PCT } from './config';
...
export async function executeSimulatedTrade(signal, marketData) {
  try {
    ...
    if (DAILY_DRAWDOWN_LIMIT_PCT > 0 && portfolio.accountValue > 0) {
      const today = new Date().toISOString().split('T')[0];

      if (portfolio.dayStartDate !== today) {
        // New day - record starting account value
        portfolio.dayStartAccountValue = portfolio.accountValue;
        portfolio.dayStartDate = today;
        await setDoc(doc(db, 'test_portfolio', 'current'), portfolio);
      }

      if (portfolio.dayStartAccountValue > 0) {
        const drawdownPct = (portfolio.dayStartAccountValue - portfolio.accountValue) / portfolio.dayStartAccountValue;
        if (drawdownPct >= DAILY_DRAWDOWN_LIMIT_PCT) {
          console.warn(`⚠️ [Test Mode] Daily drawdown limit breached: ${(drawdownPct * 100).toFixed(2)}% (limit: ${(DAILY_DRAWDOWN_LIMIT_PCT * 100).toFixed(0)}%). Forcing HOLD on all positions.`);
          return {
            success: true,
            portfolio,
            trades: [],
